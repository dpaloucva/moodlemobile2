...
/**
 * Provides a collection of contact requests.
 */
export class AddonMessagesContactRequestsSource extends CoreRoutedItemsManagerSource<AddonMessagesConversationMember> {
    ...
    protected async loadPageItems(page: number): Promise<{ items: AddonMessagesConversationMember[]; hasMoreItems: boolean }> {
        try {
            ...
            const result = await AddonMessages.getContactRequests(limitFrom, this.getPageLength());
